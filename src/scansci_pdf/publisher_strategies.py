...
from .browser_launch import camofox_launch_args
...
def _visible_camofox(config: dict[str, Any], publisher: str, *, viewport: dict | None = None):
    ...
    try:
        ctx = launch_persistent_context(
            ...
            args=camofox_launch_args(config),
        )
        page = ctx.new_page()
        ...
        browser = launch(headless=False, humanize=True, args=camofox_launch_args(config))
...
def _institution_search_terms(idp_name: str) -> list[str]:
    """Return ordered institution search/match terms for CARSI/OpenAthens WAYF pages."""
    raw = (idp_name or "").strip()
    mapped = (_IDP_MAP.get(raw, raw) or "").strip()
    terms: list[str] = []

    if raw == "南京工业大学" or mapped.lower() == "nanjing tech" or raw.lower() in {
        "nanjing tech",
        "nanjing tech university",
        "njtech",
    }:
        terms.extend(["nanjing tech", "nanjing tech university", "njtech", "南京工业大学"])
    else:
        if mapped:
            terms.append(mapped)
        if raw and raw != mapped:
            terms.append(raw)
...
_COOKIE_CONSENT_DISMISS_JS: str = """
() => {
  const visible = (el) => !!el && (
  ...
  for (const sel of selectors) {
    for (const el of document.querySelectorAll(sel)) {
      ...
      const text = (el.innerText || el.value || el.textContent || sel).trim();
      el.click();
      clicked.push(text.substring(0, 80));
      if (clicked.length >= 2) return clicked;
    }
  }
  const labels = [
    'accept all', 'accept', 'agree', 'allow all', 'consent',
    'continue', 'got it', '同意', '接受', '继续'
  ];
...
_INSTITUTION_CLICK_JS: str = (
    "async (payload) => {"
    "  const terms = (Array.isArray(payload) ? payload : [payload])"
    ...
    "    '#searchInstitution', '#bdd-email', 'input[name=\"institution\"]',"
    "    '#institution-search', 'input[placeholder*=\"institution\"]',"
    "    'input[placeholder*=\"University\"]', 'input[type=\"search\"]'"
    "  ];"
    "  let input = null;"
    "  for (const sel of searchSelectors) {"
    "    input = [...document.querySelectorAll(sel)].find(visible);"
    "    if (input) break;"
    "  }"
    "  if (input && terms[0]) {"
    "    input.focus();"
    ...
    "    fire(input, 'input');"
    "    fire(input, 'change');"
    "    fire(input, 'keydown', {key: terms[0].slice(-1) || 'h'});"
    "    fire(input, 'keyup', {key: terms[0].slice(-1) || 'h'});"
    "    await new Promise((resolve) => setTimeout(resolve, 1400));"
    "  }"
    "  const resultSelector = ["
    "    '[role=\"option\"]', '[data-testid]', '[class*=\"result\"]',"
    "    '[class*=\"suggestion\"]', '[class*=\"federation\"]',"
    "    '[class*=\"institution\"]', 'li', 'a', 'button'"
    "  ].join(', ');"
    "  const items = [...document.querySelectorAll(resultSelector)].filter(visible);"
    "  const normalize = (text) => (text || '').toLowerCase().replace(/\\s+/g, ' ').trim();"
    "  const compact = (text) => normalize(text).replace(/[\\s\\-_.(),]+/g, '');"
    "  const candidates = items"
    "    .map((el) => (el.innerText || el.value || el.textContent || '').trim())"
    "    .filter(Boolean)"
    "    .slice(0, 12);"
    "  for (const term of terms) {"
    "    const wanted = normalize(term);"
    "    const wantedCompact = compact(term);"
    "    for (const el of items) {"
    "      const text = (el.innerText || el.value || el.textContent || '').trim();"
    "      if (!text) continue;"
    "      const norm = normalize(text);"
    "      const normCompact = compact(text);"
    "      if (norm.includes(wanted) || (wantedCompact && normCompact.includes(wantedCompact))) {"
    "        const target = el.closest('a, button, [role=\"option\"], [data-testid]')"
    "          || el.querySelector('a, button') || el;"
    "        target.scrollIntoView({block: 'center', inline: 'nearest'});"
    "        target.click();"
    "        return {status: 'clicked', matchedTerm: term, text: text.substring(0, 120), input: !!input, candidates};"
    "      }"
    "    }"
    "  }"
    "  if (input) {"
    "    fire(input, 'keydown', {key: 'Enter'});"
    "    fire(input, 'keyup', {key: 'Enter'});"
    "    return {status: 'entered', input: true, candidates};"
    "  }"
    "  return {status: 'no_input', input: false, candidates};"
    "}"
)
